...
/**
* Recreates our bone cache.  Used when attaching models or changing models
*
* @return void
**/
/*void C_BaseAnimating::RecreateBoneCache(void)
{
	studiohdr_t *pStudioHdr = GetModelPtr( );

	// grab the current cache and destroy it
	int boneMask = BONE_USED_BY_HITBOX;
	CBoneCache *pcache = Studio_GetBoneCache(m_hitboxBoneCacheHandle);
	if(pcache)
	{
		// kill the current cache
		m_hitboxBoneCacheHandle = 0;
		Studio_DestroyBoneCache(m_hitboxBoneCacheHandle);
	}
	else
...
{
	int iAttachmentIndex = LookupAttachment("muzzle");
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			Vector vAttachment;
			QAngle dummyAngles;
			if (iAttachmentIndex < 0 || !GetAttachment(iAttachmentIndex, vAttachment, dummyAngles )) {
				GetAttachment( 1, vAttachment, dummyAngles );
				//DevMsg( 1, "Give %s a muzzle attachment!\n", GetClassname() );
			}
			...
			el->die = gpGlobals->curtime + 0.1f;	// was 0.05f
			el->color.r = 255;
			el->color.g = 192;
			el->color.b = 64;
			el->color.exponent = 5;
	...
	// Send out a muzzle flash
	if (iAttachmentIndex >= 0)
		tempents->MuzzleFlash( MUZZLEFLASH_SMG1, entindex(), iAttachmentIndex, false );
}
	...
	case CL_EVENT_EJECTBRASS1:
		if ( m_Attachments.Count() > 0 )
		{
			if ( MainViewOrigin().DistToSqr( GetAbsOrigin() ) < (256 * 256) )
			{
				...
				int iAttachment = LookupAttachment("eject");
...
`				GetAttachment( iAttachment+1, attachOrigin, attachAngles );
			...
			*/
			...
			tempents->MuzzleFlash( attachOrigin, attachAngles, atoi( options ), render->GetViewEntity(), bFirstPerson );
		}
		break;
...
C_BaseAnimating * C_BaseAnimating::BecomeRagdollOnClient( bool bCopyEntity )
{
	...
	// @TRJ - remove ragdolls if the game rules want us to
	if(((CSDKGameRules *)g_pGameRules)->ShouldRemoveLeftovers())
		s_RagdollLRU.MoveToTopOfLRU(pRagdoll);
...
void C_BaseAnimating::ResetSequenceInfo( void )
{
	...
	// @TRJ - might fix a crash... showed up a few times
	if(GetModelPtr())
