...
CTeamPlayHud::CTeamPlayHud( const char *pElementName ) : BaseClass(NULL, "TeamDisplay"), CHudElement( pElementName )
{
	...
	m_pWarmupLabel = new vgui::Label( this, "RoundState_warmup", "test label" /*vgui::localize()->Find( "#Clan_warmup_mode" )*/ );
...
void CTeamPlayHud::OnThink()
{
	...
	{
		wchar_t TeamName[64];
		vgui::localize()->ConvertANSIToUnicode( pTeam->Get_Name(), TeamName, sizeof(TeamName) );
		
		vgui::localize()->ConstructString( string1, sizeof(string1), vgui::localize()->Find("#Team"), 1, TeamName );
