...
void IN_WorldMapUp(void)
{
	KeyUp(&in_worldmap);

	// hide it
	if(gViewPortInterface)
		gViewPortInterface->ShowPanel(PANEL_WORLDMAP, false);
}

void IN_ChooseTeam(void)
{
	if ( gViewPortInterface )
	{
		CTeamMenu *pPanel = (CTeamMenu *)gViewPortInterface->FindPanelByName(PANEL_TEAM);
		pPanel->SetForceChooseState(false);

		gViewPortInterface->ShowPanel(pPanel, true );
	}
}

void IN_StrategyEditor(void)
{
	if(engine->IsInGame())
	{
		PRINT_CHAT("The Strategy Editor cannot be used in game!");
		return;
	}

	if(gViewPortInterface)
		gViewPortInterface->ShowPanel(PANEL_STRATEGYEDITOR, true);
}

void IN_ChooseSkill(void)
{
	if(gViewPortInterface)
	{
		CSDKPlayer *pPlayer;
		...
		if(pPlayer->GetTeamNumber() > TEAM_SPECTATOR) {
			// find the panel and display it
			CClassMenu *pPanel = (CClassMenu *)gViewPortInterface->FindPanelByName(PANEL_CLASS);
			pPanel->SetForceChooseState(false);
			gViewPortInterface->ShowPanel(pPanel, true);
		}
		else {
			PRINT_CHAT("Spectators cannot change their skill class.\n");
		}
	}
}

void IN_BuyMenu(void)
{
	if(gViewPortInterface && g_pGameRules) {
		int rv;
		if ((rv = ((CSDKGameRules*)g_pGameRules)->CanLocalPlayerBuy()) > 0)
			gViewPortInterface->ShowPanel(PANEL_BUY, true);
		else if (rv == 0) {
			PRINT_CHAT("#BuyTimeExpired");
		}
		else {
			PRINT_CHAT("#CannotBuyWhileDead");		
		}
	}
}

void IN_ChooseObjective(void)
{
	// display the panel
	if(gViewPortInterface)
...
void IN_ChooseStrategy(void)
{
	// display the panel
	if(gViewPortInterface)
		gViewPortInterface->ShowPanel(PANEL_STRATEGY, true);
}
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_WORLDMAP, s_ClearInputState, &in_worldmap, bResetState );
...
static ConCommand chooseteam("chooseteam", IN_ChooseTeam);
static ConCommand buymenu("buymenu", IN_BuyMenu);
static ConCommand chooseobjective("chooseobjective", IN_ChooseObjective);
static ConCommand choosestrategy("choosestrategy", IN_ChooseStrategy);
static ConCommand chooseskill("chooseskill", IN_ChooseSkill);
static ConCommand startworldmap("+worldmap", IN_WorldMapDown);
static ConCommand endworldmap("-worldmap", IN_WorldMapUp);
static ConCommand strategyeditor("strategyeditor", IN_StrategyEditor);
