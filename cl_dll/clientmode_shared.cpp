...
int	ClientModeShared::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	if( pPlayer && pPlayer->GetObserverMode() > OBS_MODE_DEATHCAM ) 
	{
		...
		{
			// JD: disable the spectator UI until it works
...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	else if ( Q_strcmp( "player_team", eventname ) == 0 )
	{
		...
		{
			wchar_t* wszTeamName;
			wszTeamName = vgui::localize()->Find(pTeam->Get_Name());
			if (wszTeamName) {
				char szTeamName[64];
				vgui::localize()->ConvertUnicodeToANSI(wszTeamName, szTeamName, sizeof(szTeamName));
				hudChat->Printf("Player %s joined the %s\n", pPlayer->GetPlayerName(), szTeamName);
			}
			else
		...
		{
			hudChat->Printf( "Player %s joined the %i\n", pPlayer->GetPlayerName(), team );
		}
	}
