...
#ifndef MySectionedListPanel_H
#define MySectionedListPanel_H
...
class CMySectionHeader;
class CMyItemButton;
class MySectionedListPanel;

using namespace vgui;

// sorting function, should return true if itemID1 should be displayed before itemID2
typedef bool (*MySectionSortFunc_t)(MySectionedListPanel *list, int itemID1, int itemID2);

//-----------------------------------------------------------------------------
// Purpose: List panel control that is divided up into discrete sections
//-----------------------------------------------------------------------------
class MySectionedListPanel : public Panel
{
	DECLARE_CLASS_SIMPLE( MySectionedListPanel, Panel );

public:
	MySectionedListPanel(vgui::Panel *parent, const char *name);
	~MySectionedListPanel();

	// adds a new section; returns false if section already exists
	virtual void AddSection(int sectionID, const char *name, MySectionSortFunc_t sortFunc = NULL);
	virtual void AddSection(int sectionID, const wchar_t *name, MySectionSortFunc_t sortFunc = NULL);
	...
	enum ColumnFlags_e
	{
		...
		COLUMN_FONT		= 0x20,		// render this column in a different font
	};
	...
	virtual void SetColumnFont(int sectionID, const char *columnName, const char* fontName);
	...
	*/
	...
	virtual const char *GetColumnFontBySection(int sectionID, int columnIndex);
...
private:
	...
	void AddSectionHelper(int sectionID, CMySectionHeader *header, MySectionSortFunc_t sortFunc);
	int GetSectionTall();
	...
	friend CMyItemButton;
	void SetSelectedItem(CMyItemButton *item);
	DHANDLE<CMyItemButton> m_hSelectedItem;
	...
	struct my_section_t
	{
		...
		CMySectionHeader *m_pHeader;
		CUtlVector<my_column_t> m_Columns;
		MySectionSortFunc_t m_pSortFunc;
	};

	CUtlVector<my_section_t> 				m_Sections;
	CUtlLinkedList<CMyItemButton *, int> 	m_Items;
	CUtlLinkedList<CMyItemButton *, int> 	m_FreeItems;
    CUtlVector<CMyItemButton *> 			m_SortedItems;
