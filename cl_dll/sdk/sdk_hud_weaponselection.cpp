...
void CHudWeaponSelection::Paint()
{
	...
	int iActivePos = (pSelectedWeapon ? pSelectedWeapon->GetPosition() : -1);
	...
	for ( int i = 0; i < MAX_WEAPON_SLOTS; i++ )
	{
		if ( i == iActiveSlot )
		{
			...
			for (int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++)
			{
				...
				DrawBox(xpos, ypos, largeBoxWide, largeBoxTall, selectedColor, m_flSelectionAlphaOverride, slotpos==iActivePos ? i + 1 : -1);
...
void CHudWeaponSelection::CycleToNextWeapon( void )
{
	...
	if ( pNextWeapon )
	{
		if (hud_fastswitch.GetBool() == true)//the fast weapon select command is on...
		{
			::input->MakeWeaponSelection( pNextWeapon );
		}
		else //the fast weapon select command is off
		{
