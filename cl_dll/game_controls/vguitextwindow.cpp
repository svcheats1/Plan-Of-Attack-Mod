...
//	SetTitle("", true);
	...
	//m_pOK		   = new CHiliteButton(this, "ok", "#PropertyDialog_OK");
...
void CTextWindow::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	// jump down
...
* @return Panel *
**/
Panel *CTextWindow::CreateControlByName(const char *szControlName)
{
...
void CTextWindow::OnCommand( const char *command)
{
	...
	else if(!Q_strcmp(command, "tutorial"))
	{
		// hide ourselves
		m_pViewPort->ShowPanel(this, false);

		// show the tutorial panel
		Q_snprintf(szCmd, sizeof(szCmd), "showtutorial %s", m_szExitCommand);
