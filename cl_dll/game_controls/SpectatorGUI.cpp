...
{
	// JD: disable the advanced UI until it works
	/*
...
void CSpectatorGUI::UpdateTimer()
{
	...
	// this is kind of a hack. Gets the time from the hud element.
	CHudSelfTimer *pTimer = dynamic_cast<CHudSelfTimer*>(GET_HUDELEMENT(HudRoundTimer));
	if (pTimer) {
		if (pTimer->ShouldDraw())
			_snwprintf ( szText, sizeof( szText ), L"%d:%02d\n", pTimer->DetermineMinutes(), pTimer->DetermineSeconds() );		
		else
			_snwprintf ( szText, sizeof (szText ), L"\n" );
	}
	else
		_snwprintf ( szText, sizeof (szText ), L"\n" );
