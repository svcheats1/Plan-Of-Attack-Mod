...
public:
	...
	virtual void					ForceGameRulesCheck(void);
...
{
	// kill the objective manager
	C_ObjectiveManager::Term();
...
/**
* Forces a check of the game rules so that we are guaranteed that we have the right set
*
* @return void
**/
void CHLClient::ForceGameRulesCheck(void)
{
#ifdef SUPPRESSION_ENABLED
	static int s_iSuppressionState = -1;
	CGameRulesRegister *pReg;

	// see if the suppression state is the same
	if(s_iSuppressionState != mp_suppression.GetInt())
	{
		// kill the existing rules
		if(g_pGameRules)
			delete g_pGameRules;
		g_pGameRules = NULL;

		// see if we can find the proper type
		pReg = CGameRulesRegister::FindByName(mp_suppression.GetInt() == 1 
													? "CSDKGameRulesSuppression" 
													: "CSDKGameRules");
		if(!pReg)
			Error("Couldn't force game rules check!");

		// create the object
		pReg->CreateGameRules();

		// store the state
		s_iSuppressionState = mp_suppression.GetInt();
	}
#endif
}
...
{
	// force our game rules to be checked
	ForceGameRulesCheck();
...
void CHLClient::LevelShutdown( void )
{
	...
	// destroy the skill classes
	CSkillClass::DeinitSkillClasses();
