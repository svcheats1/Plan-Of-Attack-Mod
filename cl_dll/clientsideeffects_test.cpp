...
void FX_PlayerTracer( Vector& start, Vector& end )
{
	...
	// @JDTRACER
	//VectorMA( start, TRACER_BASE_OFFSET /*+ random->RandomFloat( 0.0f, 64.0f )*/, shotDir, dStart );
	//VectorMA( dStart, ( length * 1 /*random->RandomFloat( 0.3f, 0.6f ) */ ), shotDir, dEnd );
	...
	// @TEMPTRJ - want to see the tracers longer
	t = new CFXStaticLine( "Tracer", dStart, dEnd, 1 /* random->RandomFloat( 0.5f, 0.75f ) */, 0.05f, materialName, 0 );
	assert( t );
...
void FX_Tracer( Vector& start, Vector& end, int velocity, bool makeWhiz )
{
	...
	if ( dist >= 256 )
	{
		...
		const char *materialName = "effects/spark";
		FX_AddDiscreetLine( start, dir, velocity, length, dist, random->RandomFloat( 0.75f, 0.9f ), life, materialName );
	}
