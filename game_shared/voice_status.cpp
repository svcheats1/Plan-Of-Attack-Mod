...
//char CVoiceStatus::PlayerSpeakingState( int iPlayerIndex )
/*
0	player is on other team
1	player is speaking
2	player is blocked
3	player is not blocked
*/
int CVoiceStatus::PlayerSpeakingState( int iPlayerIndex )
{
	IGameResources *gr = GameResources();
	int ret=0;
	int team = gr->GetTeam(iPlayerIndex);
	if(team != CSDKPlayer::GetLocalPlayer()->GetTeamNumber())
	{
		//player is on other team
		ret = 0;
	}
	else if(int(m_VoicePlayers[iPlayerIndex-1])==0)
	{
		//not talking and / or blocked
		if(IsPlayerBlocked(iPlayerIndex))
		{
			//blocked
			ret = 2;
		}
