...
ConVar mp_maxweapons("mp_maxweapons", "32", FCVAR_REPLICATED, "Maximum number of dropped weapons to allow");
...
bool CBaseCombatWeapon::PrecacheWeaponScriptFile( void )
{
#if defined( CLIENT_DLL )
	Assert( Q_strlen( GetClassname() ) > 0 );
	// Msg( "Client got %s\n", GetClassname() );
...
/**
* Determines the type of tracer fire to use for the current weapn
*
* @return const char *
**/
const char *CBaseCombatWeapon::GetTracerType(void)
{
	return GetWpnData().m_szTracerType;
}
...
int CBaseCombatWeapon::GetAmmo1( void ) const
{
	if (m_iPrimaryAmmoType == -1)
		return -1;
	...
	return player->GetAmmoCount(m_iPrimaryAmmoType);
}

int CBaseCombatWeapon::GetAmmo2( void ) const
{
	if (m_iSecondaryAmmoType == -1)
		return -1;
	...
	return player->GetAmmoCount(m_iSecondaryAmmoType);
}
...
void CBaseCombatWeapon::Drop( const Vector &vecVelocity )
{
	...
	// @TRJ - let the gamerules decide whether to remove it
	// JD - no, don't remove these!
	//SetRemoveable( false );
	...
	// world.
	SetRemoveable(((CSDKGameRules *)g_pGameRules)->ShouldRemoveLeftovers());
...
void CBaseCombatWeapon::SetPickupTouch( void )
{
	...
	// @TRJ - turning this back on so weapons will delete themselves
	if ( gpGlobals->maxClients > 1 && ((CSDKGameRules *)g_pGameRules)->ShouldRemoveLeftovers())
	{
		if ( GetSpawnFlags() & SF_NORESPAWN )
		{
			SetThink( &CBaseEntity::SUB_Remove );
			SetNextThink( gpGlobals->curtime + (GetAmmo1() ? 30.0f : 5.0f) );
		}
	}
...
bool CBaseCombatWeapon::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	/*
	// JD: Weapons with no ammo still need to be deployed so they can be dropped.
...
void CBaseCombatWeapon::ItemPostFrame( void )
{
	...
	if ( !bFired && (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime))
	{
		...
		if ( !IsMeleeWeapon() && !IsPrimaryEmpty() )
		{
			HandleFireOnEmpty();
		}
...
bool CBaseCombatWeapon::IsPrimaryEmpty()
{
	return UsesPrimaryAmmo() && (UsesClipsForAmmo1() ? Clip1() > 0 : HasPrimaryAmmo());
}

bool CBaseCombatWeapon::IsSecondaryEmpty()
{
	return UsesSecondaryAmmo() && (UsesClipsForAmmo2() ? Clip2() > 0 : HasSecondaryAmmo());
}
