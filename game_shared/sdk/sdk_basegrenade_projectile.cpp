...
END_NETWORK_TABLE()
	...
	void CBaseGrenadeProjectile::ResolveFlyCollisionCustom( trace_t &trace, Vector &vecVelocity )
	{
		...
		// @TRJ - what about wood.  let's do something more robust
		...
		// see if we hit glass
		breakthrough = trace.m_pEnt && trace.m_pEnt->GetCollisionGroup() == COLLISION_GROUP_BREAKABLE_GLASS;
	#ifndef CLIENT_DLL
		CBreakable *pBreakable;

		// see if it was breakable glass
		pBreakable = dynamic_cast<CBreakable *>(trace.m_pEnt);
		breakthrough = breakthrough || (pBreakable && pBreakable->GetMaterialType() == matGlass);
	#endif
