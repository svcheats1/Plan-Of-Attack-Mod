...
//-----------------------------------------------------------------------------
// Purpose: SDK specific movement code
//-----------------------------------------------------------------------------
...
// Purpose: 
//-----------------------------------------------------------------------------
inline CSDKPlayer	*CSDKGameMovement::GetSDKPlayer()
{
	return static_cast< CSDKPlayer * >( player );
}
...
//-----------------------------------------------------------------------------
inline LadderMove_t *CSDKGameMovement::GetLadderMove()
{
	CSDKPlayer *p = GetSDKPlayer();
	if ( !p )
	{
		return NULL;
	}
...
// Output : int
//-----------------------------------------------------------------------------
int CSDKGameMovement::GetCheckInterval( IntervalType_t type )
{
...
// Input  : *ladder - 
//-----------------------------------------------------------------------------
inline void CSDKGameMovement::SetLadder( CFuncLadder *ladder )
{
#ifndef CLIENT_DLL
	if ( !ladder && GetSDKPlayer()->m_Local.m_hLadder )
	{
		GetSDKPlayer()->m_Local.m_hLadder->PlayerGotOff( GetSDKPlayer() );
	}

	CSDKPlayer *pPlayer = GetSDKPlayer();
	if(pPlayer)
		pPlayer->m_Local.m_hLadder = ladder;
#endif
}
...
// Output : CFuncLadder
//-----------------------------------------------------------------------------
inline CFuncLadder *CSDKGameMovement::GetLadder()
{
	return GetSDKPlayer()->m_Local.m_hLadder;
}
...
//-----------------------------------------------------------------------------
bool CSDKGameMovement::IsForceMoveActive()
{
...
//-----------------------------------------------------------------------------
void CSDKGameMovement::SwallowUseKey()
{
	mv->m_nOldButtons |= IN_USE;
	player->m_afButtonPressed &= ~IN_USE;

	//GetSDKPlayer()->m_bPlayUseDenySound = false;
}
...
//			*ladder - 
//-----------------------------------------------------------------------------
void CSDKGameMovement::StartForcedMove( bool mounting, float transit_speed, const Vector& goalpos, CFuncLadder *ladder )
{
	LadderMove_t* lm = GetLadderMove();
	Assert( lm );
	...
	if ( ladder )
	{
		ladder->PlayerGotOn( GetSDKPlayer() );
...
//-----------------------------------------------------------------------------
bool CSDKGameMovement::ContinueForcedMove()
{
	LadderMove_t* lm = GetLadderMove();
	Assert( lm );
...
// Input  : &trace - ignored
//-----------------------------------------------------------------------------
bool CSDKGameMovement::OnLadder( trace_t &trace )
{
...
//			ladderOrigin - 
//-----------------------------------------------------------------------------
void CSDKGameMovement::Findladder( float maxdist, CFuncLadder **ppLadder, Vector& ladderOrigin, const CFuncLadder *skipLadder )
{
...
void CSDKGameMovement::GetSortedDismountNodeList( const Vector &org, float radius, CFuncLadder *ladder, CUtlRBTree< NearbyDismount_t, int >& list )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSDKGameMovement::ExitLadderViaDismountNode( CFuncLadder *ladder, bool strict )
{
...
// Input  : bOnLadder - 
//-----------------------------------------------------------------------------
void CSDKGameMovement::FullLadderMove()
{
	...
	bool neardismountnode2 = ( dist < 4.0f * 4.0f ) ? true : false;
	...
	if ( !wouldleaveladder && !neardismountnode )
	{
		...
		if ( pressed_use )
		{
			if ( ExitLadderViaDismountNode( ladder, false ) )
			{
				// See if they +used a dismount point mid-span..
				return;
			}
...
bool CSDKGameMovement::CheckLadderAutoMountEndPoint( CFuncLadder *ladder, const Vector& bestOrigin )
{
...
bool CSDKGameMovement::CheckLadderAutoMountCone( CFuncLadder *ladder, const Vector& bestOrigin, float maxAngleDelta, float maxDistToLadder )
{
#if !defined( CLIENT_DLL )
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSDKGameMovement::LookingAtLadder( CFuncLadder *ladder )
{
	if ( !ladder )
	{
		return false;
	...
	Vector vecToLadder = closest - mv->m_vecAbsOrigin;
	...
	// Facing same direction if dot > = 0.0f...
	bool facingladder = facingDot >= 0.0f;
...
// Input  : &trace - 
//-----------------------------------------------------------------------------
bool CSDKGameMovement::CheckLadderAutoMount( CFuncLadder *ladder, const Vector& bestOrigin )
{
#if !defined( CLIENT_DLL )
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CSDKGameMovement::LadderMove( void )
{
	...
	if ( ladder && 
		 ( !ladder->IsEnabled() ||
		 ( player->GetBaseVelocity().LengthSqr() > 0.0f ) ) )
	{
		GetSDKPlayer()->ExitLadder();
		ladder = NULL;
	}
#endif
