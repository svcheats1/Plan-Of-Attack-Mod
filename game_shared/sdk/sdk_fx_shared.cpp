	...
	)
{
...
#ifdef CLIENT_DLL
	C_SDKPlayer *pPlayer = dynamic_cast<C_SDKPlayer*>( ClientEntityList().GetBaseEntity( iPlayerIndex ) );
#else
	CSDKPlayer *pPlayer = dynamic_cast<CSDKPlayer*>( UTIL_PlayerByIndex( iPlayerIndex) );
#endif
	...
	if(pEvent)
	{
		...
		// set all the data
		pEvent->SetString("weapon", wpnName);
