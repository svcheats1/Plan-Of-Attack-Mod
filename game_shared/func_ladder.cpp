...
void CFuncLadder::Spawn()
{
	...
	if ( bottomtrace.startsolid || toptrace.startsolid )
	{
		...
		{
		// JD: Linux crashes when doing this?
#ifdef WIN32
			DevMsg( 1, "Warning, funcladder with blocked bottom point (%.2f %.2f %.2f) stuck in (%s)\n",
				...
				bottomtrace.m_pEnt 
					...
					"NULL" );
#endif
			// don't let buggy ladders stay in the game
			UTIL_Remove(this);
			return;
		}
...
#ifndef CLIENT_DLL
	/**
	* We need to send CFuncLadder info fairly often
	*
	* @param const CCheckTransmitInfo *pInfo
	* @return int
	**/
	int CFuncLadder::ShouldTransmit( const CCheckTransmitInfo *pInfo )
	{	
		return FL_EDICT_PVSCHECK; //FL_EDICT_ALWAYS;
	}
