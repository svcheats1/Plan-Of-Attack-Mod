...
void CGameMovement::CheckParameters( void )
{
	...
	CSDKPlayer *p = ToSDKPlayer(player);
	if ( !m_bSpeedCropped && ( mv->m_nButtons & IN_SPEED ) && !( player->m_Local.m_bDucked && !player->m_Local.m_bDucking ) && p->GetStamina() )
	{
		// @JD - SPRINT SPEED
		
		/*
		mv->m_flForwardMove *= SPRINT_RATIO;
		mv->m_flSideMove    *= SPRINT_RATIO;
		mv->m_flUpMove      *= SPRINT_RATIO;
		mv->m_flMaxSpeed	*= SPRINT_RATIO;	// evil hack..
		*/

        //mv->m_flMaxSpeed *= SPRINT_RATIO;
		//player->m_flMaxspeed *= SPRINT_RATIO;

		player->m_Local.m_bInSprint = true;
		m_bSpeedCropped = true;
	}
	else {
		player->m_Local.m_bInSprint = false;
	}
...
void CGameMovement::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMove )
{
	...
	/*
	// @TRJ - SKILLCLASS - multiply the maxspeed by the player's skillclass ratio
	CSDKPlayer *pSDKPlayer = dynamic_cast<CSDKPlayer *>(pPlayer);
	if(pSDKPlayer)
	{
		mv->m_flMaxSpeed = pSDKPlayer->MaxSpeed();
	}
	*/
...
#define PUNCH_DAMPING		20.0f // @TRJ was 9.0f		// bigger number makes the response more damped, smaller is less damped
										// currently the system will overshoot, with larger damping values it won't
#define PUNCH_SPRING_CONSTANT	100.0f // @TRJ was 65.0f	// bigger number increases the speed at which the view corrects
#define PUNCH_CUTOFF		15.0f		// note that is being compared to a squared value
#define PUNCH_CUTOFF_RATIO	0.1f
...
{
	if (player->GetMoveType() == MOVETYPE_OBSERVER)
		return;

	player->m_angViewChange.Init();

	if (player->m_Local.m_vecPunchAngle->LengthSqr() > 0.001 || player->m_Local.m_vecPunchAngleVel->LengthSqr() > 0.001)
	{
		// see if we're using a 'machine gun.'
		// This has a decent chance of returning NULL on the cast.
		CBaseMachineGun *pWpn = dynamic_cast<CBaseMachineGun*>(player->GetActiveWeapon());
		// if we're using an automatic weapon and 
		// I either released the mouse button or I can't attack anymore
		if (pWpn && pWpn->IsAutomaticWeapon() && 
			!(player->m_nButtons & IN_ATTACK) || gpGlobals->curtime > player->m_flNextAttack + 0.1) {
            
			QAngle angViewChange, angPunchNoRoll, angVecPunchNoRoll;
			
			angViewChange.Init();

			// don't include roll in this system because it causes broken neck syndrome
			angPunchNoRoll = player->m_Local.m_vecPunchAngle;
			angPunchNoRoll[ROLL] = 0;

			// if we're over the cutoff
			while (angPunchNoRoll.LengthSqr() > PUNCH_CUTOFF) {

				// move 10% over to the permanent view angle until its under the cutoff
				angViewChange += angPunchNoRoll * PUNCH_CUTOFF_RATIO;
				player->m_Local.m_vecPunchAngle -= angPunchNoRoll * PUNCH_CUTOFF_RATIO;

				// recalculate punch without roll
				angPunchNoRoll = player->m_Local.m_vecPunchAngle;
				angPunchNoRoll[ROLL] = 0;
				
				// calculate the vecpunch without roll
				angVecPunchNoRoll = player->m_Local.m_vecPunchAngleVel;
				angVecPunchNoRoll[ROLL] = 0;
                				
				player->m_Local.m_vecPunchAngleVel -= angVecPunchNoRoll * PUNCH_CUTOFF_RATIO;
			}

#ifdef CLIENT_DLL
			QAngle eyeAngles;
			engine->GetViewAngles( eyeAngles );
			VectorAdd( eyeAngles, angViewChange, eyeAngles );
...
void CGameMovement::StartGravity( void )
{
	...
	// @LADDERS
	if (player->GetGravity() || player->GetMoveType() == MOVETYPE_LADDER)
...
void CGameMovement::FullObserverMove( void )
{
	...
	// addresses sound problems
	CheckWater();

	if ( mode == OBS_MODE_IN_EYE || mode == OBS_MODE_CHASE )
	{
		CSDKPlayer * target = dynamic_cast<CSDKPlayer*>(player->GetObserverTarget());

		if ( target != NULL && target->IsAlive())
		{
			mv->m_vecAbsOrigin = target->GetAbsOrigin();
			mv->m_vecAngles = mv->m_vecViewAngles = target->GetAbsAngles();
			mv->m_vecVelocity = target->GetAbsVelocity();
#ifdef GAME_DLL
			// JD: propagate the punch angle
			player->m_Local.m_vecPunchAngle = target->m_Local.m_vecPunchAngle;
			// JD: do need to copy the velocity?
			player->m_Local.m_vecPunchAngleVel = target->m_Local.m_vecPunchAngleVel;
#endif
		}
...
void CGameMovement::CheckVelocity( void )
{
	...
	Vector vec2D;
	VectorCopy(mv->m_vecVelocity, vec2D);
	vec2D.z = 0;
	float wishspeed = VectorLength(vec2D);
	...
	if ((wishspeed != 0.0f) && (wishspeed > mv->m_flMaxSpeed))
	{
		VectorScale (vec2D, mv->m_flMaxSpeed/wishspeed, vec2D);
		mv->m_vecVelocity.x = vec2D.x;
		mv->m_vecVelocity.y = vec2D.y;
	}
}
...
void CGameMovement::PlayerMove( void )
{
	...
	switch (player->GetMoveType())
	{
		...
		case MOVETYPE_WALK:
			...
			#ifndef CLIENT_DLL
				// @TRJ - fire the footstep event if we're walking
				player->m_Local.m_fStepDistance += abs(mv->m_vecVelocity.Length2D()) * gpGlobals->frametime;
				if(player->m_Local.m_fStepDistance > STEP_THRESHOLD)
				{
					IGameEvent *pEvent = gameeventmanager->CreateEvent("player_footstep");
					if(pEvent)
					{
						// send it out
					...
					// reset the distance
					player->m_Local.m_fStepDistance = 0;
				}
			#endif
