...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	// get the tracer type
	Q_strncpy(m_szTracerType, pKeyValuesData->GetString("tracer_type", TRACER_TYPE_NONE), sizeof(m_szTracerType));
	if(!FStrEq(m_szTracerType, TRACER_TYPE_LINE) && !FStrEq(m_szTracerType, TRACER_TYPE_NONE))
		Q_strncpy(m_szTracerType, TRACER_TYPE_NONE, sizeof(m_szTracerType));
}
