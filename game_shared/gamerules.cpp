...
#define SPAWN_POINT_RADIUS 26

ConVar g_Language( "g_Language", "0", FCVAR_REPLICATED );

static CViewVectors g_DefaultViewVectors(
	Vector( 0, 0, 54 ),		// standing view height: was 64
	
	Vector(-16, -16, 0 ),	// min standing: was -16, -16, 0
	Vector( 16,  16,  72 ), // max standing: was 16, 16, 72
	
	Vector(-16, -16, 0 ),	// min ducking: was -16, -16, 0
	Vector( 16,  16,  48 ), // max ducking: was 16, 16, 36
	Vector( 0, 0, 36 ),		// ducking view height: was 28
	
	Vector(-10, -10, -10 ),	// min observer
	Vector( 10,  10,  10 ),	// max observer
	
	Vector( 0, 0, 24 )		// dead view height: was 14
);
...
CBaseEntity *CGameRules::GetPlayerSpawnSpot( CBasePlayer *pPlayer )
{
	...
	// JD: This code is from Valve
	pPlayer->AddSolidFlags(FSOLID_NOT_SOLID);
	Vector pLoc = pSpawnSpot->GetAbsOrigin() + Vector(0,0,1);
	pPlayer->Teleport(&pLoc, &pSpawnSpot->GetLocalAngles(), &vec3_origin);
	...
	/*
	JD: This is old code... keeping it just for reference
	else {
...
bool CGameRules::IsSpawnPointValid( CBaseEntity *pSpot, CBasePlayer *pPlayer  )
{
	...
	for ( CEntitySphereQuery sphere( pSpot->GetAbsOrigin(), SPAWN_POINT_RADIUS ); (ent = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
...
int CGameRules::ConnectedPlayerCount()
{
	CBasePlayer *pPlayer;
	int iCount = 0;
...
void CGameRules::Think()
{
	...
	// are we restarting?
	if(m_fRestartTime > 0 && gpGlobals->curtime > m_fRestartTime)
		// restart
		RestartGame();
}
...
/**
* Sets the time when the game needs to restart
*
* @param int iTime Number of seconds to wait before restarting
* @return void
**/
void CGameRules::SetRestartTime(int iTime)
{
	// set it
	m_fRestartTime = gpGlobals->curtime + (float)iTime; 
}
...
* @return void
**/
void CGameRules::RestartGame(void)
{
	// reset to -1
	m_fRestartTime = -1;
}
