...
void GuessDamageForce( CTakeDamageInfo *info, const Vector &vecForceDir, const Vector &vecForceOrigin, float flScale )
{
	...
	{
		CalculateBulletDamageForce( info, -1, vecForceDir, vecForceOrigin, flScale );
	}
...
float CTakeDamageInfo::GetDamage() const
{
	CSDKPlayer *pPlayer = NULL;
	CSkillClass *pSkill = NULL;
	CWeaponSDKBase *pWeapon = NULL;

	// @TRJ - SKILLCLASS - apply the damage ratio

	// get the player and the weapon
	pPlayer = dynamic_cast<CSDKPlayer *>(GetAttacker());
	pWeapon = dynamic_cast<CWeaponSDKBase *>(GetInflictor());
	if(!pPlayer || !pWeapon)
		return m_flDamage;
	...
	// figure out the damage ratio
	return m_flDamage * pSkill->GetDamageRatio(pWeapon);
}
