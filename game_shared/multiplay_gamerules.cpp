...
#include <KeyValues.h>
	...
	#include "iscorer.h"
	#include "sdk_player.h" // @TRJ - sorry
...
bool CMultiplayRules::IsMultiplayer( void )
{
	...
	CMultiplayRules::CMultiplayRules()
	{
		...
		// no messages yet
		m_bFiveMinuteWarning = false;
		m_bOneMinuteWarning = false;
	}
	...
	/**
	* Determines if the intermission has ended
	*
	* @return bool
	**/
	bool CMultiplayRules::IntermissionOver(void)
	{
		return gpGlobals->curtime >= m_flIntermissionEndTime;
	}
	...
	void CMultiplayRules::Think ( void )
	{
		...
		if ( g_fGameOver && IntermissionOver())   // someone else quit the game already
		{
			ChangeLevel(); // intermission is over
			return;
		}

		// are we within five minutes of the end of the game?
		if(!m_bFiveMinuteWarning && (fEndTime - gpGlobals->curtime) < 300)
		{
			UTIL_SayTextAll("Five minutes remaining!\n");
			m_bFiveMinuteWarning = true;
		}
		// are we within one minute of the end of the game?
		if(!m_bOneMinuteWarning && (fEndTime - gpGlobals->curtime) < 60)
		{
			UTIL_SayTextAll("One minute remaining!\n");
	...
	bool CMultiplayRules::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBaseCombatWeapon *pWeapon )
	{
		...
		if (FStrEq(engine->GetClientConVarValue( pPlayer->entindex(), "cl_autowepswitch" ), "0")) //cl_autowepswitch is not checked off
		{
			return false;
		}
	...
	int CMultiplayRules::IFragsForKill( CBasePlayer *pAttacker, CBasePlayer *pKilled )
	{
		return 1;
	}
	...
	void CMultiplayRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		if ( pVictim == pScorer )  
		{
			...
			pVictim->IncrementScore( -1 );
		}
		else if ( pScorer )
		{
			// if a player dies in a deathmatch game and the killer is a client, award the killer some points
			pScorer->IncrementScore( IPointsForKill( pScorer, pVictim ) );
			pScorer->IncrementFragCount( IFragsForKill( pScorer, pVictim ) );
	...
	void CMultiplayRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		{
			// Is the killer a client? <- (sounds like a personal problem)
			if ( pScorer )
			{
				killer_ID = pScorer->GetUserID();
		...
		if(pVictim->GetUserID() == killer_ID && strncmp(killer_weapon_name,"grenade_projectile",13) != 0 && strncmp(killer_weapon_name,"rpg_missile",11) != 0)
			killer_weapon_name="none";
		...
		if ( event )
		{
			...
			// @TRJ - need the skills
			CSDKPlayer *pAttacker = ToSDKPlayer(pScorer);
			CSDKPlayer *pPlayer = ToSDKPlayer(pVictim);
			if(pAttacker && pPlayer)
			{
				CSkillClass *pAttackerSkill = pAttacker->GetSkillClass();
				CSkillClass *pPlayerSkill = pPlayer->GetSkillClass();
				event->SetString("attacker_skill", pAttackerSkill->GetInternalClassName());
				event->SetString("skill", pPlayerSkill->GetInternalClassName());
			}
	...
	* @return void
	**/
	void CMultiplayRules::RestartGame(void)
	{
		// @TRJ - don't reset the player again or we will take away all of their things
		// at this point they have already spawned...
