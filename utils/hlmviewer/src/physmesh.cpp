...
{
	char phyName[512];
	Q_strncpy( phyName, pFilename, sizeof( phyName ) );
	Q_SetExtension( phyName, ".phy", sizeof( phyName ) );

	FileHandle_t fp = g_pFileSystem->Open( phyName, "rb" );
	...
	int pos = g_pFileSystem->Tell( fp );
	g_pFileSystem->Seek( fp, 0, FILESYSTEM_SEEK_TAIL );
	int fileSize = g_pFileSystem->Tell(fp) - pos;
	g_pFileSystem->Seek( fp, pos, FILESYSTEM_SEEK_HEAD );

	char *buf = (char *)_alloca( fileSize );
	g_pFileSystem->Read( buf, fileSize, fp );
