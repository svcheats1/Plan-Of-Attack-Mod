...
CUtlDict< studiohdr_t *, int >	g_LoadedGroupFiles;

void StudioModel::UnloadGroupFiles()
{
	g_pCacheHdr = NULL;

	int c = g_LoadedGroupFiles.Count();
	int i;
	for ( i = 0; i < c; i++ )
	{
		delete[] (unsigned char *)g_LoadedGroupFiles[ i ];
	}
	g_LoadedGroupFiles.RemoveAll();
}

studiohdr_t *FindOrLoadGroupFile( char const *modelname )
{
	int idx = g_LoadedGroupFiles.Find( modelname );
	if ( idx != g_LoadedGroupFiles.InvalidIndex() )
	{
		return g_LoadedGroupFiles[ idx ];
	}

	FileHandle_t fp;

	// load the model
	if( (fp = g_pFileSystem->Open( modelname, "rb" )) == NULL)
	{
		return NULL;
	}
	...
	idx = g_LoadedGroupFiles.Insert( modelname, ( studiohdr_t * )buffer );
	return g_LoadedGroupFiles[ idx ];
}
...
int StudioModel::BoneMask( void )
{
	...
	if (g_viewerSettings.showAttachments || g_viewerSettings.m_iEditAttachment != -1 || m_nSolveHeadTurn != 0)
	{
...
void StudioModel::SetUpBones( bool mergeBones )
{
	...
	mstudioseqdesc_t	&seqdesc = m_pstudiohdr->pSeqdesc( m_sequence );

	QAngle a1;
	Vector p1;
	MatrixAngles( g_viewtransform, a1, p1 );
	CIKContext *pIK = NULL;
	m_ik.Init( m_pstudiohdr, a1, p1, GetRealtimeTime(), m_iFramecounter, BoneMask( ) );
	...
	AccumulatePose( m_pstudiohdr, pIK, pos, q, m_sequence, m_cycle, m_poseparameter, BoneMask( ), 1.0, GetRealtimeTime() );

	if ( g_viewerSettings.blendSequenceChanges &&
		m_sequencetime < m_blendtime && 
		m_prevsequence != m_sequence &&
		m_prevsequence < m_pstudiohdr->GetNumSeq() &&
		!(seqdesc.flags & STUDIO_SNAP) )
	{
		...
		AccumulatePose( m_pstudiohdr, NULL, pos, q, m_prevsequence, m_prevcycle, m_poseparameter, BoneMask( ), s, GetRealtimeTime() );
	...
	for (j = 0; j <= iMaxPriority; j++)
	{
		for (i = 0; i < MAXSTUDIOANIMLAYERS; i++)
		{
			...
			{
				AccumulatePose( m_pstudiohdr, pIK, pos, q, m_Layer[i].m_sequence, m_Layer[i].m_cycle, m_poseparameter, BoneMask( ), m_Layer[i].m_weight, GetRealtimeTime() );
			}
		}
	}
	...
	CIKContext auto_ik;
	auto_ik.Init( m_pstudiohdr, a1, p1, 0.0, 0, BoneMask( ) );
	...
	if (pIK)
	{
		...
		pIK->UpdateTargets( pos, q, m_pStudioRender->GetBoneToWorld(0), boneComputed );
		...
		pIK->SolveDependencies( pos, q, m_pStudioRender->GetBoneToWorld(0), boneComputed );
	}
	...
	for (i = 0; i < m_pstudiohdr->numbones; i++) 
	{
		if ( !(m_pstudiohdr->pBone( i )->flags & BoneMask()))
		{
			...
			for (j = 0; j < 3; j++)
			{
				for (k = 0; k < 4; k++)
				{
					(*m_pStudioRender->GetBoneToWorld( i ))[j][k] = VEC_T_NAN;
				}
			}
			continue;
		}
		...
		else if (CalcProceduralBone( m_pstudiohdr, i, CBoneAccessor( m_pStudioRender->GetBoneToWorldArray() ) ))
		{
			continue;
		}
		else
		...
		if (!mergeBones)
		{
			...
			MatrixCopy( *m_pStudioRender->GetBoneToWorld( i ), boneCache[i] );
		}
		else if (g_pCacheHdr)
		{
			for (j = 0; j < g_pCacheHdr->numbones; j++)
			{
				if (stricmp( m_pstudiohdr->pBone( i )->pszName(), g_pCacheHdr->pBone( j )->pszName() ) == 0)
				{
					break;
				}
			}
			...
			{
				MatrixCopy( boneCache[j], *m_pStudioRender->GetBoneToWorld( i ) );
			}
		}
	}
...
//	g_viewerSettings.lightrot.y = fmod( (90 * GetTickCount( ) / 1000.0), 360.0);
	...
	m_pStudioRender->SetLocalLights( 2, light );
#endif
	...
	m_pStudioRender->SetAmbientLightColors( m_AmbientLightColors );
}
...
void StudioModel::UpdateStudioRenderConfig( bool bFlat, bool bWireframe, bool bNormals )
{
	...
	if( g_viewerSettings.renderMode == RM_FLATSHADED || g_viewerSettings.renderMode == RM_SMOOTHSHADED )
	{
...
void StudioModel::DrawAttachments( )
{
	...
	{
		mstudioattachment_t &pattachments = (mstudioattachment_t &)m_pstudiohdr->pAttachment( i );

		matrix3x4_t world;
		ConcatTransforms( *m_pStudioRender->GetBoneToWorld( m_pstudiohdr->GetAttachmentBone( i ) ), pattachments.local, world );
...
void StudioModel::DrawEditAttachment()
{
	...
	if ( iEditAttachment >= 0 && iEditAttachment < m_pstudiohdr->GetNumAttachments() )
	{
		g_pMaterialSystem->Bind( g_materialBones );
		
		mstudioattachment_t &pAttachment = (mstudioattachment_t &)m_pstudiohdr->pAttachment( iEditAttachment );

		matrix3x4_t world;
		ConcatTransforms( *m_pStudioRender->GetBoneToWorld( m_pstudiohdr->GetAttachmentBone( iEditAttachment ) ), pAttachment.local, world );
...
void StudioModel::DrawIllumPosition( )
{
	...
	VectorCopy(m_pstudiohdr->illumposition, modelPt0);
	VectorCopy(m_pstudiohdr->illumposition, modelPt1);
	modelPt0.x -= 4;
	modelPt1.x += 4;
...
void StudioModel::SetViewTarget( void )
{
	...
	mstudioattachment_t &patt = (mstudioattachment_t &)m_pstudiohdr->pAttachment( iEyeAttachment );
	matrix3x4_t attToWorld;
	ConcatTransforms( *m_pStudioRender->GetBoneToWorld( m_pstudiohdr->GetAttachmentBone( iEyeAttachment ) ), patt.local, attToWorld ); 
...
void StudioModel::SetHeadPosition( Vector pos[], Quaternion q[] )
{
	...
	matrix3x4_t attToWorld;
	int iBone =  m_pstudiohdr->GetAttachmentBone( iEyeAttachment );
	BuildBoneChain( m_pstudiohdr, g_viewtransform, pos, q, iBone, m_pStudioRender->GetBoneToWorld( 0 ) );
	ConcatTransforms( *m_pStudioRender->GetBoneToWorld( iBone ), patt.local, attToWorld );
...
int StudioModel::DrawModel( bool mergeBones )
{
	...
	UpdateStudioRenderConfig( g_viewerSettings.renderMode == RM_FLATSHADED ||
			...
			false ); // garymcthack - should really only do this once a frame and at init time.
	...
	for (i = 0; i < m_pstudiohdr->numflexdesc; i++)
	{
	...
	m_pStudioRender->SetFlexWeights( MAXSTUDIOFLEXDESC, g_flexdescweight, g_flexdescweight2 );
	...
	g_DrawModelInfo.m_pStudioHdr = m_pstudiohdr;
	g_DrawModelInfo.m_pHardwareData = &m_HardwareData;
	...
	if( g_viewerSettings.renderMode == RM_BONEWEIGHTS )
	{
		...
		DebugDrawModelBoneWeights( m_pStudioRender, g_DrawModelInfo, vecModelOrigin, &m_LodUsed, &m_LodMetric );
	}
	else
	{
		count = m_pStudioRender->DrawModel( g_DrawModelInfo, vecModelOrigin, &m_LodUsed, &m_LodMetric );
		m_pStudioRender->GetPerfStats( g_DrawModelInfo, NULL );
	}

	if( g_viewerSettings.showNormals )
	{
		DebugDrawModelNormals( m_pStudioRender, g_DrawModelInfo, vecModelOrigin, &m_LodUsed, &m_LodMetric );
		DebugDrawModelTangentS( m_pStudioRender, g_DrawModelInfo, vecModelOrigin, &m_LodUsed, &m_LodMetric );
		DebugDrawModelTangentT( m_pStudioRender, g_DrawModelInfo, vecModelOrigin, &m_LodUsed, &m_LodMetric );
	}
	...
	if (g_viewerSettings.showShadow)
	{
		matrix3x4_t invViewTransform;
		...
		for (int i = 0; i < m_pstudiohdr->numbones; i++) 
		{
			matrix3x4_t *pMatrix = m_pStudioRender->GetBoneToWorld( i );
		...
		g_DrawModelInfo.m_Lod = m_HardwareData.m_NumLODs - 1;

		float zero[4] = { 0, 0, 0, 0 };
		m_pStudioRender->SetColorModulation( zero );
		m_pStudioRender->ForcedMaterialOverride( g_materialShadow );
		m_pStudioRender->DrawModel( g_DrawModelInfo, vecModelOrigin, NULL, NULL );
		m_pStudioRender->ForcedMaterialOverride( NULL );
		float one[4] = { 1, 1, 1, 1 };
		m_pStudioRender->SetColorModulation( one );
	}
