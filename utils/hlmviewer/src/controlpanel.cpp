...
: mxWindow (parent, 0, 0, 0, 0, "Control Panel", mxWindow::Normal)
{
	...
	cbShadow = new mxCheckBox (wRender, 110, 105, 150, 20, "Shadow", IDC_SHADOW);
	...
	cbNormalMap = new mxCheckBox (wRender, 5, 105, 100, 20, "Normal Mapping", IDC_NORMALMAP);
	...
	cbSpecular = new mxCheckBox (wRender, 5, 145, 100, 20, "Specular", IDC_SPECULAR);
	...
	cbIllumPosition = new mxCheckBox (wRender, 260, 105, 150, 20, "Illum. Position", IDC_ILLUMPOSITION);
...
LoadModelResult_t ControlPanel::loadModel(const char *filename)
{
	...
	if (g_pStudioModel->LoadModel( filename ))
	{
		if (g_pStudioModel->PostLoadModel( filename ))
		{
			...
			if (!LoadViewerSettings( filename, g_pStudioModel ))
			{
				InitViewerSettings( "hlmv" );
				centerView();
				setSequence( 0 );
	...
	for (i = 0; i < 4; i++)
	{
		...
		{
			g_pStudioExtraModel[i]->FreeModel();
...
LoadModelResult_t ControlPanel::loadModel(const char *filename, int slot )
{
	...
	{
		g_pStudioExtraModel[slot]->FreeModel();
	}
...
ControlPanel::initSequenceChoices()
{
	...
	if (hdr)
	{
		for (int i = 0; i < MAX_SEQUENCES; i++)
		{
			...
			{
				Assert( j < ARRAYSIZE(iSequenceToSelection) );
				Assert( k < ARRAYSIZE(iSelectionToSequence) );

				if (g_viewerSettings.showHidden || !(hdr->pSeqdesc(j).flags & STUDIO_HIDDEN))
				{
					if (g_viewerSettings.showActivities)
					{
						cSequence[i]->add( hdr->pSeqdesc(j).pszActivityName() );
					}
					else
					{
						cSequence[i]->add( hdr->pSeqdesc(j).pszLabel() );
					}
					iSelectionToSequence[k] = j;
					iSequenceToSelection[j] = k;
					k++;
				}
				else
				{
					// previous valid selection
					iSequenceToSelection[j] = (k > 0) ? (k - 1) : 0;
				}
			}
...
ControlPanel::setModelInfo ()
{
	...
	if( checkSum == hdr->checksum && boneLODCount == g_DrawModelInfo.m_NumHardwareBones && numBatches == g_DrawModelInfo.m_NumBatches)
	{
		return;
	}
