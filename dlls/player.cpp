...
ConVar mp_basearmor("mp_basearmor", "100", FCVAR_NOTIFY);
...
bool CBasePlayer::WantsLagCompensationOnEntity( const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	...
	if ( !friendlyfire.GetBool() && pPlayer->GetTeamNumber() == GetTeamNumber() )
		return false;
...
void CBasePlayer::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
	if ( m_takedamage )
	{
		...
		case HITGROUP_GENERIC:
			DevMsg("HitGroup Generic\n");
			break;
		case HITGROUP_HEAD:
			info.ScaleDamage( 3.0 );
			DevMsg("HitGroup Head\n");
			break;
		case HITGROUP_CHEST:
			info.ScaleDamage( 1.15 );
			DevMsg("HitGroup Chest\n");
			break;
		case HITGROUP_STOMACH:
			info.ScaleDamage( 1.0 );
			DevMsg("HitGroup Stomach\n");
			break;
...
// New values
#define ARMOR_RATIO	0.8
#define ARMOR_BONUS	3.0
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	flBonus = ARMOR_BONUS;
	// armor is more effective if you have more of it.
	flRatio = (1 - (min(m_ArmorValue / 100.0, 2.0) * (1 - ARMOR_RATIO)));
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( event )
	{
		...
		{
			// pull their current weapon
			CSDKPlayer *pPlayerInflictor = ToSDKPlayer(pInflictor);
			CBaseCombatWeapon *pWeapon = pPlayerInflictor->GetActiveWeapon();
			if(pWeapon)
				event->SetString("weapon", STRING(pWeapon->m_iClassname));
		}
		else
			event->SetString("weapon", STRING(pInflictor->m_iClassname));

		if ( attacker->IsPlayer() )
		{
			// @TRJ
			//CBasePlayer *player = ToBasePlayer( attacker );
			CSDKPlayer *player = ToSDKPlayer( attacker );
			event->SetInt("attacker", player->GetUserID() ); // hurt by other player

			// add the skill info
			CSkillClass *pSkill = player->GetSkillClass();
			event->SetString("skill", pSkill->GetInternalClassName());
		}
		else
		{
			// @TRJ
			event->SetString("skill", "none");
...
void CBasePlayer::WaterMove()
{
	...
	*/
	...
	// you don't have to be alive to be under water
	if ( GetWaterLevel() < 3 )
	{
		if ( m_bPlayerUnderwater )
		{
	...
	// you have to be alive to wade
	else if ( GetWaterLevel() < 2 && IsAlive())
	{
		if ( GetWaterLevel() == 0 )
		{
	...
	// you don't have to be alive to be above water
	...
	// if we're dead, don't do anything else.
	if (!IsAlive())
		return;
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if ( HasWeapons() )
	{
		// JD: This doesn't actually do anything, the weapon gets dropped from 
		// CBaseCombatCharacter::Event_Killed()
		// At this point that has already been called.
		...
		*/
		...
		// But we do still need to remove all the items from the player.
		RemoveAllItems(true);
	}
...
// choose to respawn.
	...
	{
		// @TRJ - make sure they have control if they were coming out of deathcam
		...
		// nothing else to do, so stop thinking
...
void CBasePlayer::StopObserverMode()
{
	...
	m_Local.m_iHideHUD &= ~HIDEHUD_OBSERVING;
	m_Local.m_iHideHUD &= ~HIDEHUD_PLAYERDEAD;
...
bool CBasePlayer::StartObserverMode(int mode)
{
	...
	// has health, yes. This is how gamemovement determines if you're dead.
	m_iHealth = 1;	

	// dead, yes. don't let players follow me or draw me.
	m_lifeState = LIFE_DEAD;
	pl.deadflag = true;

	// send new HUD flags etc
	UpdateClientData();
...
bool CBasePlayer::SetObserverMode(int mode )
{
	...
	{
		// otherwise, obey the force cam setting
		switch ( ((CTeamplayRules *)g_pGameRules)->GetForceCam() )
		{
	...
	if (  m_iObserverMode == OBS_MODE_IN_EYE || m_iObserverMode == OBS_MODE_CHASE )
	{
		if ( !IsValidObserverTarget( m_hObserverTarget.Get() ) )
		{
			...
			else
			{
				...
				if ( ((CTeamplayRules *)g_pGameRules)->GetForceCam() == OBS_ALLOW_ALL )
				{
		...
		if ( target && m_iObserverMode == OBS_MODE_IN_EYE )
		{
			...
			m_Local.m_bDrawViewmodel = false;
	}
}
	...
	else if ( m_iObserverMode == OBS_MODE_IN_EYE )
	{
		...
		if ( target->IsPlayer() )
		{
			...
			for ( int i = 0; i < MAX_VIEWMODELS; i++ )
			{
				...
				// turn off the viewmodels we have already
				if (m_hViewModel[i])
					m_hViewModel[i]->AddEffects( EF_NODRAW );
			}
...
void CBasePlayer::Spawn( void )
{
 ...
 // only preserve the shadow flag
	...
	m_bitsDamageType = 0;
	m_bitsHUDDamage = -1;
	//m_bPlayerUnderwater = false;  // @JD @WATER problem
...
// Suicide...
void CBasePlayer::CommitSuicide(bool bForce/* = false*/)
{
	if( !IsAlive() )
		return;
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	// ----------------------------------------
	CBaseCombatWeapon *pMyWeapon = Weapon_OwnsThisType( pWeapon->GetClassname(), pWeapon->GetSubType());
	if (pMyWeapon) 
	{
		// @JD - This is kind of a hack since it should be CSDKPlayer instead. OTOH,
		// if valve had not been such a bitch and put all these functions in
		// the player and put them in the weapon instead I wouldn't even be here.
		CWeaponSDKBase *pWpn = dynamic_cast<CWeaponSDKBase*>(pMyWeapon);

		if (pWpn) {
			if (pWpn->EquipAmmoFromWeapon(pWeapon)) {
				UTIL_Remove(pWeapon);
                return true;
			}
			return false;
...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_SERVERCLASS_ST( CBasePlayer, DT_BasePlayer )
		...
		SendPropInt		(SENDINFO(m_ArmorValue), 10),
...
/**
* Resets everything for the current player as if we just joined the game
*
* @return void
**/
void CBasePlayer::Reset(void)
{
	// reset the frag and death counts
	ResetFragCount();
	ResetDeathCount();
	ResetScore();

	// reset the health
	TakeHealth(GetMaxHealth() - GetHealth(), DMG_GENERIC);
}
