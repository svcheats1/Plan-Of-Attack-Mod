...
void CHLTVDirector::FireGameEvent( IGameEvent * event )
{
	...
	if(strncmp(gameevent.m_Event->GetName(), "objective_selected", 18) == 0)
	{
	}
...
void CHLTVDirector::SetHLTVServer( IHLTVServer *hltv )
{
	...
	if ( hltv ) 
	{
		...
		gameeventmanager->AddListener( this, "objective_selected", true);
	}
	else
	{
...
void CHLTVDirector::CreateShotFromEvent( CGameEvent *event )
{
	...
	if ( !Q_strcmp( "objective_selected", name ) )
	{
		CBaseEntity *picker = UTIL_PlayerByUserId( event->m_Event->GetInt("userid") );
		if(!picker)
			return;
		shot = gameeventmanager->CreateEvent("hltv_chase",true);
		shot->SetInt("target2",picker->entindex());
		shot->SetInt( "theta", 40 );
		shot->SetInt( "phi", -30 );
	}
	else if ( !Q_strcmp( "player_death", name ) ||
