...
void ClientKill( edict_t *pEdict )
{
	...
	CSDKPlayer *pl = (CSDKPlayer*) GetContainingEntity( pEdict );
	...
	// if we're frozen ignore them
	if(pGameRules->IsPlayerFrozen(pl))
		return;
	*/
	pl->CommitSuicide();
}
...
void Host_Say( edict_t *pEdict, bool teamonly )
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		// don't allow dead people to talk to alive people
		if ( !mp_canheardeadpeople.GetBool() && bSenderDead && client->m_lifeState == LIFE_ALIVE )
			continue;

		// Allow dead people to see what alive people say
		/*
		if ( !bSenderDead && client->m_lifeState != LIFE_ALIVE )
			continue;
		*/
...
static ConCommand fov("fov", CC_Player_FOV, "Change players FOV", FCVAR_CHEAT);
...
* Restarts the game
**/
CON_COMMAND_F( mp_restartgame, "Sets the number of seconds until the game is restarted", FCVAR_GAMEDLL )
{
	// make sure we got a time
	if( engine->Cmd_Argc() == 2 )
	{
		// restart
		g_pGameRules->SetRestartTime(atoi(engine->Cmd_Argv(1)));
	}
}