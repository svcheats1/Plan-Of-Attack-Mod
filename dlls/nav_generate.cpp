...
void CNavArea::ComputeApproachAreas( void )
{
	...
	FOR_EACH_LL( TheNavAreaList, it )
	{
		...
		if (NavAreaBuildPath( farArea, this, NULL, cost ) == false)
			continue;
		...
		while( m_approachCount < MAX_APPROACH_AREAS )
		{
			...
			int count = 0;
			CNavArea *area;
			for( area = this; area; area = area->GetParent() )
				++count;
			...
			int i = 0;
			for( area = this; i < count && area; area = area->GetParent() )
				path[ i++ ] = area;
	...
	switch( m_generationState )
	{
		...
		case FIND_HIDING_SPOTS:
		{
			while( m_generationIndex != TheNavAreaList.InvalidIndex() )
			{
				...
				{
					Msg( "Finding hiding spots...%d%%\n", 100 * m_generationIndex / TheNavAreaList.Count() );
					return true;
				}
			}
		...
		case FIND_EARLIEST_OCCUPY_TIMES:
		{
			while( m_generationIndex != TheNavAreaList.InvalidIndex() )
			{
				...
				if( Plat_FloatTime() - startTime > maxTime )
				{
					Msg( "Finding earliest occupy times...%d%%\n", 100 * m_generationIndex / TheNavAreaList.Count() );
					return true;
				}
			}
