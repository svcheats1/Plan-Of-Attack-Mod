...
void FinishClientPutInServer( CSDKPlayer *pPlayer )
{
	...
	if (!pPlayer->IsBot())
	{
		// When the player first joins the server, they need to pick a team
...
void GameStartFrame( void )
{
	...
	// any chance we restarted? install new game rules
	if(g_bGameRestarted)
	{
		// Set up game rules
		if (g_pGameRules)
		...
		// install the game rules
		InstallGameRules();
		g_pGameRules->RestartGame();

		// make sure we don't try to restart again
...
void InstallGameRules()
{
	...
	if(mp_suppression.GetInt() > 0)
		CreateGameRulesObject("CSDKGameRulesSuppression");
	else
#endif
