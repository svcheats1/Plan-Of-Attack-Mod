...
ConVar nav_dark_colors( "nav_dark_colors", "0", FCVAR_GAMEDLL, "Uses dark line colors when editing bright areas." );
...
void NavDrawLine( const Vector& from, const Vector& to, NavEditColor navColor )
{
	...
	if ( nav_dark_colors.GetBool() && ( navColor == NavCursorColor || navColor == NavSplitLineColor ) )
	{
		color = Color( 0, 0, 1 );
	}
