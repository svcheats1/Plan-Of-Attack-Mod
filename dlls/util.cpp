...
/**
* Sends the tip to the specified user
*
* @param const char *szTip The tip to send
* @param CBasePlayer *pPlayer The player to send the tip to 
* @param const char *szReplacements The replacments for the first sublabel.  : separated
* @return void
**/
void UTIL_SendTip(const char *szTip, CBasePlayer *pPlayer, const char *szReplacements/* = NULL*/)
{
	// check that we got them
	if(!pPlayer)
		return;

	// create the filter and pass the buck
	CSingleUserRecipientFilter sFilter(pPlayer);
	sFilter.MakeReliable();
	UTIL_SendTipFilter(szTip, sFilter);
}

/**
* Sends the tip to the specified team
*
* @param const char *szTip The tip to send
* @param int iTeam The team to send the tip to
* @param const char *szReplacements The replacments for the first sublabel.  : separated
* @return void
**/
void UTIL_SendTipTeam(const char *szTip, int iTeam, const char *szReplacements/* = NULL*/)
{
	// check that we got them
	...
	// create the filter and pass the buck
	CTeamRecipientFilter sFilter(iTeam, true);
	UTIL_SendTipFilter(szTip, sFilter);
}

/**
* Sends the tip to all the players
*
* @param const char *szTip The tip to send
* @param const char *szReplacements The replacments for the first sublabel.  : separated
* @return void
**/
void UTIL_SendTipAll(const char *szTip, const char *szReplacements/* = NULL*/)
{
	// create the filter and pass the buck
	CReliableBroadcastRecipientFilter sFilter;
	UTIL_SendTipFilter(szTip, sFilter);
}

/**
* Sends the tip using the specified filter
*
* @param const char *szTip The tip to send
* @param IRecipientFilter &sFilter The filter to use
* @param const char *szReplacements The replacments for the first sublabel.  : separated
* @return void
**/
void UTIL_SendTipFilter(const char *szTip, IRecipientFilter &sFilter, const char *szReplacements/* = NULL*/)
{
	int iReplacements = 0;
	char *szStr, *szCopy;

	// send the message
	UserMessageBegin(sFilter, "HudDisplayMsg");

	// tip header, no replacements
	WRITE_STRING("Tip");
	WRITE_SHORT(0);
	...
	// bail if we don't have any replacements
	if(!szReplacements)
	{
		// no replacments, no second sublabel
		WRITE_SHORT(0);
		WRITE_BYTE(0);
		MessageEnd();
		return;
	...
	szCopy = new char[strlen(szReplacements) + 1];
	Q_strncpy(szCopy, szReplacements, strlen(szReplacements) + 1);

	// figure out the number of replacements
	szStr = strtok(szCopy, ":");
	while(szStr)
	{
	...
	// kill the string cause we just messed it up
	delete [] szCopy;
	szCopy = new char[strlen(szReplacements) + 1];
	Q_strncpy(szCopy, szReplacements, strlen(szReplacements) + 1);

	// send the replacments
	WRITE_SHORT(iReplacements);
	szStr = strtok(szCopy, ":");
	while(szStr)
	{
		// send the string and move on
		WRITE_STRING(szStr);
		szStr = strtok(NULL, ":");
	}
	delete [] szCopy;
...
{
	if (!pToPlayer || !pToPlayer->IsNetClient() )
		return;
...
/**
* Sends a message to the players team
*
* @param const char *pText The message to send
* @param CBasePlayer *pEntity The entity sending the message
* @param bool bChat Chat?
* @return void
**/
void UTIL_SayTextTeam( const char *pText, CBasePlayer *pEntity, bool bChat )
{
	CTeam *pTeam;

	// get the team
	pTeam = pEntity->GetTeam();

	// create the filter
	CTeamRecipientFilter filter(pTeam->GetTeamNumber(), true);

	// send it
	UTIL_SayTextFilter(filter, pText, pEntity, bChat );
}
...
static void SetMinMaxSize (CBaseEntity *pEnt, const Vector& mins, const Vector& maxs )
{
	for ( int i=0 ; i<3 ; i++ )
	{
		if ( mins[i] > maxs[i] )
		{
			...
			// @JD: maybe we should bail when this happens, since the server crashes?
			return;
		}
	}
