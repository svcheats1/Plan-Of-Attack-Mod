...
void SendProxy_PlayerList( const SendProp *pProp, const void *pStruct, const void *pData, DVariant *pOut, int iElement, int objectID )
{
	...
	Assert( iElement < pTeam->GetPlayers()->Size() );

	CBasePlayer *pPlayer = (*(pTeam->GetPlayers()))[iElement];
...
IMPLEMENT_SERVERCLASS_ST_NOBASE(CTeam, DT_Team)
	...
	SendPropBool(SENDINFO(m_bIsOffensive)),
END_SEND_TABLE()
...
{
	Assert( m_aPlayers.IsValidIndex(iIndex) );

	if ( !m_aPlayers.IsValidIndex(iIndex) )
		return NULL;
...
CBasePlayer *CTeam::GetRandomPlayer()
{
	return GetPlayer(random->RandomInt( 0, m_aPlayers.Count() - 1 ));
}
...
/**
* Determines if this is an 'active' team.  Active means that the 
* team is part of regular game play (A or B)
*
* @return bool
**/
bool CTeam::IsPlayingTeam(void)
{
	// are we A or B?
