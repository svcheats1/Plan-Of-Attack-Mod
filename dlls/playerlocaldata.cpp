...
BEGIN_SEND_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	SendPropBool	(SENDINFO(m_bInSprint)),
...
//	SendPropInt		(SENDINFO(m_nOldButtons),	22, SPROP_UNSIGNED ),
	...
	SendPropBool(SENDINFO_STRUCTELEM(m_LadderMove.m_bForceLadderMove)),
	SendPropBool(SENDINFO_STRUCTELEM(m_LadderMove.m_bForceMount)),
	SendPropFloat(SENDINFO_STRUCTELEM(m_LadderMove.m_flStartTime), 0, SPROP_NOSCALE),
	SendPropFloat(SENDINFO_STRUCTELEM(m_LadderMove.m_flArrivalTime), 0, SPROP_NOSCALE),
	SendPropVector(SENDINFO_STRUCTELEM(m_LadderMove.m_vecGoalPosition), -1,  SPROP_COORD),
	SendPropVector(SENDINFO_STRUCTELEM(m_LadderMove.m_vecStartPosition), -1, SPROP_COORD),
	SendPropEHandle(SENDINFO_STRUCTELEM(m_LadderMove.m_hForceLadder)),
	SendPropEHandle(SENDINFO_STRUCTELEM(m_LadderMove.m_hReservedSpot)),
END_SEND_TABLE()
...
BEGIN_DATADESC_NO_BASE( LadderMove_t )
	...
	DEFINE_FIELD(m_flArrivalTime, FIELD_FLOAT),
	DEFINE_FIELD(m_vecGoalPosition, FIELD_VECTOR),
	DEFINE_FIELD(m_vecStartPosition, FIELD_VECTOR),
