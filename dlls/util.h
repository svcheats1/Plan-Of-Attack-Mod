...
*
* @param const char *szList[] The list to search in.  Last element assumed to be ""
* @param const char *szItem The item to find
* @return bool True if it's in the list
**/
inline bool FindInList(const char *szList[], const char *szItem)
{
	// flip through until we find ""
	for(int i = 0; !FStrEq(szList[i], ""); ++i)
	{
		if(FStrEq(szItem, szList[i]))
			return true;
	}
...
void		UTIL_SayTextTeam( const char *pText, CBasePlayer *pEntity, bool bChat = false );
