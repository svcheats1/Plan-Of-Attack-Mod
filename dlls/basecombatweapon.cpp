...
extern ConVar mp_maxweapons;

CGlobalWeaponManager *CGlobalWeaponManager::s_pInstance = NULL;

/**
* Access the singleton instance
*
* @return CGlobalWeaponManager *
**/
CGlobalWeaponManager *CGlobalWeaponManager::GetInstance(void)
{
	// create the instance if we don't have it
	if(!s_pInstance)
		s_pInstance = new CGlobalWeaponManager();
...
* @return void
**/
void CGlobalWeaponManager::Term(void)
{
	// kill the singleton
	if(s_pInstance)
		delete s_pInstance;
...
* Constructor
**/
CGlobalWeaponManager::CGlobalWeaponManager()
{
	// reset ourselves
	Reset();
}
...
* @return void
**/
void CGlobalWeaponManager::Reset(void)
{
	// clean immediately
	m_fNextCleanTime = gpGlobals->curtime;
}

/**
* Cleans up weapons if we're over our max
*
* @return void
**/
void CGlobalWeaponManager::CleanWeapons(void)
{
	int iFreeWeaponCount = 0;
	CBaseCombatWeapon *pWeapon;

	// only do this every so often and don't bother if we're not removing
	if(gpGlobals->curtime < m_fNextCleanTime 
		|| !((CSDKGameRules *)g_pGameRules)->ShouldRemoveLeftovers()
		|| !mp_maxweapons.GetInt())
		return;

	// wait a second before cleaning again
	m_fNextCleanTime = gpGlobals->curtime + 2.0f;

	// run through all the weapons in the map and try to remove them
	pWeapon = (CBaseCombatWeapon *)gEntList.FindEntityByClassname(NULL, "weapon_*");
	while(pWeapon)
	{
		...
		// if we're over our limit we can start deleting as long as no one can see us
		if(mp_maxweapons.GetInt() < iFreeWeaponCount && !UTIL_FindClientInPVS(pWeapon->edict()))
		{
			// kill it
		...
		// move along
		pWeapon = (CBaseCombatWeapon *)gEntList.FindEntityByClassname(pWeapon, "weapon_*");
	}

	// are under?
	if(iFreeWeaponCount < mp_maxweapons.GetInt())
		return;

	// still over? delete ones people can see
	iFreeWeaponCount = 0;
	pWeapon = (CBaseCombatWeapon *)gEntList.FindEntityByClassname(NULL, "weapon_*");
	while(pWeapon)
	{
		...
		// if we're over our limit we can start deleting as long as no one can see us
		if(mp_maxweapons.GetInt() < iFreeWeaponCount)
		{
			// kill it
			pWeapon->AddEffects(EF_NODRAW);
			UTIL_Remove(pWeapon);
