...
const char * KeyValues::ReadToken( char **buffer, bool &wasQuoted )
{
	...
	if ( c == '\"' )
	{
		...
		while ( true )
		{
			...
			if ( c == 0 )
			{
				g_KeyValuesErrorStack.ReportError( "ReadToken unexpected EOF in quoted string" );
				return NULL;
			}

			if ( c == '\n' || c == '\r' )
			{
				g_KeyValuesErrorStack.ReportError( "unexpected CR/LF in string" );
				break;
			}
			...
			if ( c == '\\' && m_bHasEscapeSequences )
			{
				...
				switch ( c )
				{
					case 0  :	g_KeyValuesErrorStack.ReportError( "ReadToken unexpected EOF after \\" );
								return NULL;
					...
					default:   
								g_KeyValuesErrorStack.ReportError( "ReadToken illegal character after \\" );
...
/**
* Sets the sub key of the current key
*
* @param KeyValues *pData The key to add as the child
* @return void
**/
void KeyValues::SetSubKey(KeyValues *pDat)
{
	Assert(!m_pSub);
	m_pSub = pDat;
}
...
/**
* Determines the last key in the list
*
* @return KeyValues * The last value in the m_pPeer list
**/
KeyValues *KeyValues::GetLastKey(void)
{
	KeyValues *pLast;

	// loop through until we can loop no further
	pLast = this;
	while(pLast->GetNextKey())
...
{
	SetColor(FindKey(keyName, true), value);
}

void KeyValues::SetColor(int keySymbol, Color value)
{
	SetColor(FindKey(keySymbol), value);
}

void KeyValues::SetColor(Color value)
{
	SetColor(this, value);
}

void KeyValues::SetColor(KeyValues *dat, Color value)
{
	if ( dat )
	{
...
void KeyValues::SetString(int keySymbol, const char *value )
{
	SetString(FindKey(keySymbol), value);
}

void KeyValues::SetString( const char *keyName, const char *value )
{
	SetString(FindKey(keyName, true), value);
}
...
void KeyValues::SetString(KeyValues *dat, const char *value)
{
	if ( dat )
	{
...
void KeyValues::SetWString(int keySymbol, const wchar_t *value )
{
	SetWString(FindKey(keySymbol), value);
}

void KeyValues::SetWString( const char *keyName, const wchar_t *value )
{
	SetWString(FindKey(keyName, true), value);
}

void KeyValues::SetWString(const wchar_t *value )
{
	SetWString(this, value);
}

void KeyValues::SetWString(KeyValues *dat, const wchar_t *value)
{
	if ( dat )
	{
		// delete the old value
...
void KeyValues::SetInt(int keySymbol, int value )
{
	SetInt(FindKey(keySymbol), value);
}

void KeyValues::SetInt( const char *keyName, int value )
{
	SetInt(FindKey(keyName, true), value);
}
...
void KeyValues::SetInt(KeyValues *dat, int value)
{
	if ( dat )
	{
		dat->m_iValue = value;
...
void KeyValues::SetFloat(int keySymbol, float value )
{
	SetFloat(FindKey(keySymbol), value);
}

void KeyValues::SetFloat( const char *keyName, float value )
{
	SetFloat(FindKey(keyName, true), value);
}

void KeyValues::SetFloat(float value )
{
	SetFloat(this, value);
}

void KeyValues::SetFloat(KeyValues *dat, float value)
{
	if ( dat )
	{
		dat->m_flValue = value;
...
//-----------------------------------------------------------------------------
void KeyValues::SetPtr(int keySymbol, void *value )
{
	SetPtr(FindKey(keySymbol), value);
}

void KeyValues::SetPtr( const char *keyName, void *value )
{
	SetPtr(FindKey(keyName, true), value);
}

void KeyValues::SetPtr(void *value )
{
	SetPtr(this, value);
}

void KeyValues::SetPtr(KeyValues *dat, void *value)
{
	if ( dat )
	{
		dat->m_pValue = value;
