...
static bool ReadFile( char const* pFileName, CTempImage& image, int maxbytes = -1 )
{
	...
	fileHandle = s_pFileSystem->Open( pFileName, "rb" );
	if( !fileHandle )
		return false;
	...
	if (maxbytes < 0)
	{
		pos = s_pFileSystem->Size( fileHandle );
	}
	else
	{
		pos = maxbytes;
	...
	s_pFileSystem->Seek( fileHandle, 0, FILESYSTEM_SEEK_HEAD );

	// Read the file into the vector memory
	int len = s_pFileSystem->Read( image.Base(), pos, fileHandle );

	// Close the file
	s_pFileSystem->Close( fileHandle );
	...
	// Read the file into the vector memory
	int len = fread( image.Base(), 1, pos, fp );
...
static bool ReadSourceImage( CUtlBuffer& buf, TGAHeader_t& header, CTempImage& image )
{
	...
	int memRequired = ImageLoader::GetMemRequired( header.width, header.height, 
		...
		int height, ImageFormat imageFormat, float targetGamma, bool mipmap )
{
	...
	ImageLoader::GenerateMipmapLevels( tmpImage.Base(), pOutputImage,
		header.width, header.height, imageFormat, ARTWORK_GAMMA, targetGamma,
		mipmap ? 0 : 1 );
			...
			ImageFormat imageFormat, float targetGamma, bool mipmap )
{
	...
	CUtlBuffer buf( vec.Base(), vec.NumAllocated() );
...
//-----------------------------------------------------------------------------
static bool LoadRGBA8888( CUtlBuffer& buf, CUtlMemory<unsigned char> &outputData, int &outWidth, int &outHeight )
{
	TGAHeader_t header;
	...
	int memSize = ImageLoader::GetMemRequired( 
		header.width, header.height, IMAGE_FORMAT_RGBA8888, false );
